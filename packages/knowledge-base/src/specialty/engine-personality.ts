...
const CDIR = join(__dirname, 'content');
const DIM = 'personality';
...
export function analyzePersonality(ctx:SharedContext):AnalysisItem[]{
  ...
  function pick(prefix:string,s:number,o:number):any{const strong=s>totalScore*0.1;const t=elIsJi(di,o,ctx)?J():Y();const key=prefix+(strong?'_strong':'_weak');return t[key]||t[prefix];}
