...
export function analyzeParents(ctx:SharedContext):AnalysisItem[]{
  ...
  if(ctx.parentsPalace.isJiShen){ const r=rules.yearBad; if(r)items.push({level:'确定',layer1:r.l1||'',layer2:r.l2||'',layer3:r.l3||''}); }
  if(ctx.seals.strength==='强'||ctx.seals.strength==='一般'){ const r=rules.sealStrong; if(r)items.push({level:'确定',layer1:r.l1||'',layer2:r.l2||'',layer3:r.l3||''}); }
  if(ctx.wealthStars.strength==='强'){ const r=rules.wealthStrong; if(r)items.push({level:'确定',layer1:r.l1||'',layer2:r.l2||'',layer3:r.l3||''}); }
  return items;
}
